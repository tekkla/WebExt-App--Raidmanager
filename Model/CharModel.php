...
final class CharModel extends Model
{
    ...
    final protected function extendChar(&$char)
    {
        ...
        // The edit button
        $actionbar->createButton('edit', 'ajax', 'icon')->setRoute('raidmanager_char_edit', $param);

        // Delete button only on non mainchars
        if ($char->is_main == 0)
            $actionbar->createButton('delete', 'ajax', 'icon')->setRoute('raidmanager_char_delete', $param);
    ...
    public function checkNameExists()
    {
        ...
        if (isset($this->data->id_char))
        {
            ...
            $param['id_char'] = $this->data->id_char;
        }
